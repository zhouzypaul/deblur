...
def deblur(y):
    ...
    for y in reversed(image_pyramid):
        ...
        k, x = estimate_blur_kernel(y, k, kernel_size)

        x = threshold_image(x)
        latent_imgs.append(x)

    # perform final deconvolve using final estimated kernel
    # final_deblurred = solve_latent(y, k)
    # k = solve_kernel(x, y, hp.lmda, kernel_size)
    k, final_deblurred = estimate_blur_kernel(y, k, kernel_size)
...
def estimate_blur_kernel(y, k, kernel_size):
    """
...
def solve_kernel(latent_img, blur_img, weight, psf_size):
    """
    this function estimates the blur kernel using an interative process as
    outlined in Algorithm 2 of the paper
    args:
