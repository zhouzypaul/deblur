...
def deblur(y):
    ...
    """
    kernel_size = (15, 15) if len(y.shape) == 2 else (15, 15, y.shape[2])
...
def estimate_latent(blur_img, kernel):
    latent = blur_img
    n, m, c = np.atleast_3d(blur_img).shape

    # precompute
    fx = psf2otf(np.array([[1, -1]]), (n, m))
    fy = psf2otf(np.array([[1, -1]]).T, (n, m))
    ker = psf2otf(kernel, (n, m))
    nablas = np.abs(fx) ** 2 + np.abs(fy) ** 2

    if c > 1:
        nablas = expand_color_channel(nablas, c)
...
def estimate_kernel(latent_img, blur_img, weight, psf_size):
    _, _, c = np.atleast_3d(latent_img).shape
    ...
    if c > 1:
        dx = expand_color_channel(dx, c)
        dy = expand_color_channel(dy, c)

    lxf = fft2(convolve(latent_img, dx))
    lyf = fft2(convolve(latent_img, dy))

    bxf = fft2(convolve(blur_img, dx))
    byf = fft2(convolve(blur_img, dy))
...
def solve_u(latent, beta):
    _, _, c = np.atleast_3d(latent).shape
    threshold = hp.lmda * hp.sigma / beta
    if c > 1:
        condition = np.sum(latent ** 2, axis=-1) >= threshold
        condition = expand_color_channel(condition, c)
    else:
        condition = latent ** 2 >= threshold
    return np.where(condition, latent, 0)

def solve_g(h, v, miu, c):
    if c > 1:
        condition = np.sum(h ** 2 + v ** 2, axis=-1) >= hp.lmda / miu
        condition = expand_color_channel(condition, c)
    else:
...
def compute_fg(latent, miu):
    ...
    v = np.vstack((np.diff(latent, axis=0), v_diff[None, :]))
    gh, gv = solve_g(h, v, miu, c)

    # compute horizontal and vertical components in FG
    gh_diff = gh[:, -1] - gh[:, 0]
    h_component = np.hstack((gh_diff[:, None], -np.diff(gh, axis=1)))
    gv_diff = gv[-1, :] - gv[0, :]
    v_component = np.vstack((gv_diff[None, :], -np.diff(gv, axis=0)))
